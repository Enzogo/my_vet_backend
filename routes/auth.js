...
function signToken(userDoc) {
  // IMPORTANTE: incluir sub (lo espera el middleware)
  const payload = {
...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    await session.withTransaction(async () => {
      const created = await User.create([{ email, passwordHash, role: roleNorm, nombre: nombre || '' }], { session })
      userDoc = created[0]
    })
    ...
    return res.status(201).json({
      token,
      user: { id: userDoc._id.toString(), email: userDoc.email, role: userDoc.role, nombre: userDoc.nombre || null }
    })
  } catch (err) {
    console.error('[MyVet] /api/auth/register error:', err)
...
router.post('/login', async (req, res) => {
  try {
    ...
    console.error('[MyVet] /api/auth/login error:', err)
    return res.status(500).json({ message: 'Error interno' })
  }
})
